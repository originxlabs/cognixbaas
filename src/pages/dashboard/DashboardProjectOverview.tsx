...
const DashboardProjectOverview = () => {
  ...
  const generationStep = currentProject.generation_step || 'prompt';
  const steps = [
    { id: 'prompt', label: 'Requirements', done: generationStep !== 'prompt' },
    { id: 'analysis', label: 'Analysis', done: ['tasks', 'architecture', 'apis', 'database', 'sandbox'].includes(generationStep) },
    { id: 'tasks', label: 'Tasks', done: ['architecture', 'apis', 'database', 'sandbox'].includes(generationStep) },
    { id: 'architecture', label: 'Architecture', done: ['apis', 'database', 'sandbox'].includes(generationStep) },
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                <span>Backend: <span className="text-foreground">{currentProject.backend_type}</span></span>
                <span>Database: <span className="text-foreground">{currentProject.database_type}</span></span>
              </div>
            </div>
          ...
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" asChild>
                <Link to={`/dashboard/project/${currentProject.id}/prompt`} className="gap-1">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-foreground mt-1 line-clamp-3">{currentProject.prompt}</p>
                </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-foreground mt-1">{currentProject.auth_method || 'JWT + Refresh Token'}</p>
                </div>
                {currentProject.payments && (
                  <div>
                    <span className="text-xs text-muted-foreground uppercase tracking-wider">Payments</span>
                    <p className="text-sm text-foreground mt-1">{currentProject.payments}</p>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <Link to={`/dashboard/project/${currentProject.id}/tasks`}>
        ...
        <Link to={`/dashboard/project/${currentProject.id}/apis`}>
          <Card className="bg-card/50 border-border hover:border-primary/50 transition-colors cursor-pointer h-full">
            <CardContent className="p-4 flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
        ...
        <Link to={`/dashboard/project/${currentProject.id}/sandbox`}>
          <Card className="bg-card/50 border-border hover:border-primary/50 transition-colors cursor-pointer h-full">
            <CardContent className="p-4 flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
